...
// Only create test suite if we have a valid reseller
if (!reseller) {
    console.error(`❌ ERROR: No valid reseller found for ACTION="${action}"`);
    console.error(`Available resellers: ${(resellerConfig?.resellers || []).map(r => r.name).join(', ')}`);
    
    // Create a failing test to show the error in Jenkins
    test.describe('Configuration Error', () => {
        test('should have valid ACTION parameter', () => {
            throw new Error(`No valid reseller found for ACTION="${action}". Check your resellers.json config.`);
        });
    });
} else {
    console.log(`✅ Running tests for reseller: ${reseller.name}`);
