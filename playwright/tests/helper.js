...
async function checkElementExists(page, locator, name, timeout = 20000) {
  try {
    ...
    // Wait until at least one element is attached (exists in DOM)
    ...
    if (!isVisible) {
      logError(`❌ ${name} exists but is not visible. Locator: ${locator}`);
      return { name, status: "hidden" };
    }

    expect(isVisible).toBeTruthy(); // Assert visible for test framework
    logSuccess(`✅ ${name} exists and is visible.`);
    return { name, status: "visible" };
...
async function titleDetailsScreen(page,reseller) {
    logStep('Checking for the title on the Details screen...');
    try {
        let titleLocator;
        const titleSelector = reseller.titleDetailsScreen;
...
async function backgroundImageDetailsScreen(page,reseller) {
    logStep('Checking for background images in the Details screen...');
    try {
        let imageLocator;
        // Use the backgroundImageDetailsScreen from reseller config, with default fallback
        const imageSelector = reseller.backgroundImageDetailsScreen || 'div.details-image.details-image-desktop';
        imageLocator = imageSelector.includes('img.img-fluid')
            ? page.locator(imageSelector).nth(1)
            : page.locator(imageSelector);
...
async function buttonsDetailsScreen(page, reseller, loggedUser) {
    ...
    try {
       ...
       await page.waitForFunction(() =>
        ...
        if (reseller.name !== 'panamsport' && reseller.name !== 'prtv' && reseller.name !== 'tdmax' && reseller.name !== 'tvin' && reseller.name !== 'gamestreammedia') {
        ...
        let buttons;
        if (reseller.name === 'panamsport' && loggedUser || reseller.name === 'gols' && loggedUser || reseller.name === 'prtv' && !loggedUser || reseller.name === 'prtv' && loggedUser || reseller.name === 'tdmax'
            && loggedUser || reseller.name === 'tvin' && loggedUser || reseller.name === 'gamestreammedia' && loggedUser
        ) {
        ...
        for (let i = 0; i < buttonCount; i++) {
            ...
            if (
            (reseller.name === 'prtv' && (lowerText.includes('watch now') || lowerText.includes('ver ahora'))) ||
            (loggedUser && lowerText.includes('watch now') && reseller.name === 'panamsport') ||
            ((reseller.name === 'gols' || reseller.name === 'tvin' || reseller.name === 'gamestreammedia') && loggedUser && lowerText.includes('watch now')) || 
            (reseller.name === 'tdmax' && (lowerText.includes('watch now') || lowerText.includes('ver ahora')))
            ){
                ...
                if ( reseller.name === 'gols' && loggedUser) {
                    return;
                }
                watchNowFound = true;
                    // Here we don't check player for gols becuase they Deleted and some vods will not work
                if ( loggedUser && reseller.name !== 'gols' || !loggedUser && reseller.name === 'prtv' || loggedUser && reseller.name === 'tdmax') {
            ...
            if (['amorir', 'okgol', 'televicentro','panamsport','gols','gamestreammedia','flexflix','tvin','fearpix'].includes(reseller.name) &&
                (trimmedText.toLowerCase() === 'suscribirse' || trimmedText.toLowerCase() === 'subscribe' || trimmedText.toLowerCase().includes('buy now') ||
                 trimmedText.toLowerCase().includes('watch for free'))) {
                const isVisible = await button.isVisible();
                const isEnabled = await button.isEnabled();
                ...
                }else if (reseller.name === 'gamestreammedia' && !loggedUser){
                    suscribirseFound = true;
                    logSuccess(`✅ Found "Watch for Free" button at Details screen`);
                }else{
                ...
                try {
                    ...
                    }else if (reseller.name === 'flexflix' && !loggedUser) {
                    ...
                    try {
                        ...
                        await page.waitForTimeout(3000); // If you expect a popup, wait for it or use waitForSelector
                        await checkSharePopup(page, 'cdk-overlay-0');
                    } catch (error) {
                        await page.screenshot({ path: `share_button_not_found_${reseller.name}.png` });
                    ...
                    await page.screenshot({ path: `suscribirse_button_click_error_${reseller.name}.png` });
        ...
        if (!watchNowFound && reseller.name === 'prtv' || !watchNowFound && loggedUser && reseller.name === 'panamsport' || !watchNowFound && loggedUser && reseller.name === 'gols' ||
            !watchNowFound && loggedUser && reseller.name === 'tdmax' || !watchNowFound && loggedUser && reseller.name === 'tvin' || !watchNowFound && loggedUser && reseller.name === 'gamestreammedia') {
            const msg = `❌ "Watch Now" button not found for action: ${reseller.name}`;
            await page.screenshot({ path: `watch_now_button_not_found_${reseller.name}.png` });
            logError(msg);
            throw new Error(msg);
        }
        if (['amorir', 'okgol', 'televicentro','panamsport','gols','gamestreammedia','flexflix','tvin','fearpix'].includes(reseller.name) && !suscribirseFound) {
            await page.screenshot({ path: `suscribirse_button_not_found_${reseller.name}.png` });
            const msg = `❌ "Suscribirse" button not found for action: ${reseller.name}`;
            logError(msg);
            throw new Error(msg);
        }
    } catch (err) {
...
async function checkPlayerScreen(page, reseller, stepNumber, value) {
    await test.step(`${stepNumber}. Checking Player screen...`, async () => {
        ...
        try {
            ...
            if (reseller.name !== 'prtv' && reseller.name !== 'tvin'){
...
async function checkSharePopup(page, popupId) {
    try {
        ...
        if (isVisible) {
            ...
            // Wait for the image to be attached to DOM (give it time to load)
            try {
                await imageLocator.first().waitFor({ state: 'attached', timeout: 20000 });
                const imageCount = await imageLocator.count();
                ...
                console.log("⚠️  Image element not found or took too long to load:", err.message);
                await page.screenshot({ path: `share_popup_image_missing_${Date.now()}.png` });
                logError('Image element does not exist or failed to load.');
                throw new Error('Image element does not exist.');
            }
...
async function checkDinamiclyPopUP(page, reseller, selector) {
    try {
...
async function logOutUser(page,reseller) {
