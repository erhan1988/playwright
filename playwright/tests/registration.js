...
async function registrationScreen(page, reseller, stepNumber) {
  ...
  logStep(`${stepNumber}. Registration Screen Different Scenario: 1. Scenario check if exist all fields`);
    ...
    if ( reseller.name === 'gols'){
      await page.waitForTimeout(4000); // or better: wait for a container element
    }
   // if (reseller.name !== 'okgol' && reseller.name !== 'televicentro' && reseller.name !== 'tvin' && reseller.name !== 'fearpix') {
    if (reseller.registrationButtonHeaderExist !== false) {
     ...
     const loginButton = page.getByRole('button', { name: /Log In|Iniciar sesión|Ingresar/ });
      ...
      if (await loginButton.count()) {
        try {
          ...
          console.error("❌ Login button found in DOM but never became visible");
          await page.screenshot({ path: 'login_button_not_visible.png', fullPage: true });
          throw e;
        }
      } else {
        throw new Error("❌ Login button not found at all!");
      }
    ...
    const h1 = page.locator('h1');
      ...
      if (reseller.name === 'emmanuel') {
        ...
        // ];
      }else if (reseller.name === 'amorir' || reseller.name === 'gols' || reseller.name === 'gamestreammedia') {
        requiredFields = [
          { locator: '#firstname', name: 'First Name' },
          { locator: '#lastname', name: 'Last Name' },
          { locator: '#email', name: 'Email' },
      ...
      }else if (reseller.name === 'prtv' || reseller.name === 'flexflix'){
        requiredFields = [
          { locator: '#firstname', name: 'First Name' },
          { locator: '#lastname', name: 'Last Name' },
      ...
      }else if (reseller.name === 'okgol' || reseller.name === 'televicentro' || reseller.name === 'tvin' || reseller.name === 'fearpix') {
          requiredFields = [
            { locator: '#firstname', name: 'First Name' },
            { locator: '#email', name: 'Email' },
            { locator: '#password', name: 'Password' },
      ...
      }else if ( reseller.name === 'panamsport') {
          requiredFields = [
            { locator: '#email', name: 'Email' },
            { locator: '#password', name: 'Password' },
            { locator: '#confirmPassword', name: 'Confirm Password' },
      ...
      await checkSubscribeButtonDisabled(page, reseller);
      console.log('All fields are empty. Now checking if the subscribe button is disabled...');

      stepNumber += 1;
      await regScreenSecondScenario(page, reseller, stepNumber);

      stepNumber += 1;
      await regScreenThirdcenario(page, reseller, stepNumber);

      stepNumber += 1;
      await regScreenFourcenario(page, reseller, stepNumber);

      stepNumber += 1;
      await regScreenFivecenario(page, reseller, stepNumber);

      if(reseller.name === 'tdmax'){
        stepNumber +=1;
       await landingPageRegisterButton (page,reseller,stepNumber)
      }

      stepNumber += 1;
      await regScreenSixcenario(page, reseller, stepNumber);
...
async function regScreenSecondScenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Registration Screen Second Scenario: Need to appear warning message in Email , Password and Confirm Password`, async () => {
    ...
    try {
       ...
       // Fill First Name 
       if (reseller.register?.firstName){
        await page.locator('#firstname').fill(reseller.register.firstName);
        const firstName = await page.locator('#firstname').inputValue();
        console.log(`First Name has Value: ${firstName}`);
       }

       //Fill Last Name
      if (reseller.register?.lastName) {
        await page.locator('#lastname').fill(reseller.register.lastName);
        const lastName = await page.locator('#lastname').inputValue();
        console.log(`Last Name has Value: ${lastName}`);
      }

       //Fill Phone 
      if (reseller.register?.phone) {
        await page.locator('#phone').fill(reseller.register.phone);
      ...
      await page.locator('#confirmPassword').fill(reseller.register.confirmPassNotValid);
      ...
      await termsOfUseCheckBox(page, reseller, false);
...
async function regScreenThirdcenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Registration Screen Third Scenario: Need to appear warning message in Confirm Password all others to be filled`, async () => {
    ...
    try {
      ...
      // Fill Email Valid
      await page.locator('#email').fill(reseller.register.email);
      const email = await page.locator('#email').inputValue();
      console.log(`Email has Value: ${email}`);
      ...
      // Fill Password  Valid
      await page.locator('#password').fill(reseller.register.password);
      const password = await page.locator('#password').inputValue();
      console.log(`Password has Value: ${password}`);
...
async function regScreenFourcenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Registration Screen Four Scenario: Filled all fields only checkbox not checked button Submit need to be Disable `, async () => {
    ...
    try {
      ...
      await page.locator('#confirmPassword').fill(reseller.register.confirmPassword);
      const confirmPassword = await page.locator('#confirmPassword').inputValue();  
      console.log(`Confirm Password has  Value: ${confirmPassword}`);
      await page.locator('#confirmPassword').blur();
...
async function regScreenFivecenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Registration Screen Five Scenario:Create new User success `, async () => {
    ...
    try {
      ...
      //Refresh the page to reset the form
        ...
        await page.waitForSelector('#subscribe-button', { state: 'visible', timeout: 15000 });
     ...
     // Fill Email 
      ...
      await termsOfUseCheckBox(page, reseller,true);

      // Check if button subscribe is disabled
      console.log('Now checking if the subscribe button is enabled...');
      await checkSubscribeButtonDisabled(page, reseller,'enabled');

      await redirectionAfterRegistration(page, reseller);
...
async function regScreenSixcenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Registration Screen Six Scenario:Try to create new User with already exist email `, async () => {
    ...
    try {
      ...
      if (reseller.name === 'okgol'|| reseller.name === 'televicentro' || reseller.name === 'tvin' || reseller.name === 'fearpix') {
         const baseUrl = `https://${reseller.name}-v3-dev.streann.tech/subscribe`;
         await page.goto(baseUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });      // Wait for the login screen to load
      }else{
        // CLick to redirect to Registration Screen from the Header
        await page.waitForTimeout(5000); // Wait for 5 seconds
      const subscribeButton = page.getByText(/subscribe now|subscribe|suscr[ií]base ahora|¡hazte miembro!|register|reg[íi]strate gratis/i, { exact: false });       
      if (await subscribeButton.count()) {
          await subscribeButton.first().click();
          console.log("Subscribe button clicked in the header!");
        } else {
...
async function termsOfUseCheckBox(page, reseller, checkedCheckbox) {
...
async function checkSubscribeButtonDisabled(page, reseller, enabled) {
    if (enabled) {
      // Should be enabled
...
async function redirectionAfterRegistration(page, reseller) {
  try {
    if  (reseller.choosePlanRedirectRegistration) {
        ...
        }else{await expect(page.locator('h1.d-flex.justify-content-center.text-center.mt-1.title.mb-1.pt-1')).toHaveText(/Elige Tu Plan|Choose Your Plan/i, { timeout: 10000 });
        }
  ...
  }else if (reseller.name === 'emmanuel') {
       // Check here if will Appear pop up for email verification
      await emailVerificationPopup(page,reseller.name);
  }else if (!reseller.choosePlanRedirectRegistration) {
      const url = `https://${reseller.name}-v3-dev.streann.tech/`;
