...
async function checkCategoryTitleHomeScreen(page,reseller,stepNumber, options = {}) {
    await test.step(`${stepNumber}. Check the Home screen and print titles of all categories`, async () => {
        ...
        try {
            ...
            await page.waitForSelector(reseller.titleCategoryHomeScreen, { timeout: 5000 });

            // Find all matching elements
            const h5Elements = await page.locator(reseller.titleCategoryHomeScreen).all();
...
async function checkVodsInHome(page, reseller, stepNumber) {
    await test.step(`${stepNumber}. Check if images exist and click the appropriate VOD`, async () => {
        ...
        try {
            ...
            if (reseller.name === 'televicentro' ) {
            ...
            }else if (reseller.name === 'tdmax' || reseller.name === 'flexflix' || reseller.name === 'amorir'){
                if (imageCount >= 58) {
                    imageToClickIndex = 58; // click the first available VOD
                    console.log('ℹ️ Tdmax,flexflix,Amorir detected. Clicking the 58 available VOD.');
                } else {
                ...
                // Default behavior
                if (imageCount >= 15) {
                    imageToClickIndex = 12;
                }else if (reseller.name === 'okgol') {
                    imageToClickIndex = 8;
                }
...
async function UserDetailsScreen(page,reseller,stepNumber,loggedUser) {
    ...
    await test.step(`${stepNumber}. ${userType} checking Details screen contains Go back Title background Image Buttons`, async () => {
        ...
        try {
            ...
            await titleDetailsScreen(page,reseller);

            //Background image check
            await backgroundImageDetailsScreen(page,reseller);

            // Buttons in the Details screen
            await buttonsDetailsScreen(page,reseller,loggedUser);
...
async function checkRelatedContentInDetailsScreen(page, reseller, stepNumber) {
    await test.step(`${stepNumber}. Check Related Content in Details screen`, async () => {
      logStep(`${stepNumber}. Checking related content in the Details screen...`);
        try {
            // Skip if buttonRelatedContent is not defined in config
            if (!reseller.buttonRelatedContent) {
                logError(`❌ buttonRelatedContent not configured for ${reseller.name}`);
                throw new Error(`❌ buttonRelatedContent not configured for ${reseller.name}`);
            }

            let titleButtons;
            let count = 0;

            titleButtons = await page.locator(reseller.buttonRelatedContent);
           ...
           // await imageToClick.click();
            ...
            const newUrl = page.url();
            logSuccess(`✅ Successfully redirected to: ${newUrl}`);
            if (reseller.name !== 'panamsport' && reseller.name !== 'prtv') {
                await page.waitForTimeout(11000);
            }else if (reseller.name === 'prtv'){
