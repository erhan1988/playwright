...
async function checkFooterLinks(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Check Footer links elements are exists`, async () => {
    ...
    try {
      if (reseller.name === 'panamsport' || reseller.name === 'gols') {
      ...
      await checkTextExist(page, [
        reseller.termsOfUseText
      ]);

      // 🛑 Now move to privacy policy **only after above check passes**
      await privacyPolicy(page,reseller);

      // CHECK CONTACT US
      if (reseller.name !== 'amorir'){
          await contactUs(page,reseller);
      }
    } catch (err) {
...
async function privacyPolicy(page,reseller) {
  try {
    ...
    privacyLinkText = await privacyLink.textContent();
    await logSuccess(`Found Privacy Policy link: "${privacyLinkText?.trim()}"`);
    ...
    const privacyText = reseller.privacyText;
...
async function contactUs(page, reseller) {
  try {
    const contactUsText = reseller.contactUsText ;
    const locatorType = reseller.contactUsLocatorType;

    let contactLink;
    if (locatorType === 'link') {
      contactLink = page.locator('a', { hasText: new RegExp(contactUsText, 'i') }).first();
      await expect(contactLink).toBeVisible();
      console.log('Contact Us link href:', await contactLink.getAttribute('href'));
    } else {
      contactLink = page.getByText(new RegExp(contactUsText, 'i'), { exact: false }).first();
    }
    await expect(contactLink).toBeVisible();
    const contactLinkText = await contactLink.textContent();
    await logSuccess(`Found Contact Us link: "${contactLinkText?.trim()}"`);

    if (reseller.name !== 'okgol' && reseller.name !== 'gols') {
      await Promise.all([
        contactLink.click(),
