...
async function contactUsFirstScenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Contact Us Different Scenario: 1. First Scenario Check if Title and fields exist`, async () => {
    ...
    try {
      ...
      stepNumber += 1;
      await contactUsSecondScenario(page, reseller, stepNumber);

      // Increment step number and call the third scenario as a separate step
      stepNumber += 1;
      await contactUsThirdScenario(page, reseller, stepNumber);

      // Increment step number and call the four scenario as a separate step
      stepNumber += 1;
      await contactUsFourthScenario(page, reseller, stepNumber);
...
async function contactUsSecondScenario(page, reseller, stepNumber) {
...
async function contactUsThirdScenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Contact Us Third Scenario: Validate warning messages for Email and Text Area`, async () => {
    ...
    try {
      ...
      await page.locator('#subject').fill(reseller.contactUs?.contactUsSubject);
...
async function contactUsFourthScenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Contact Us Fourth Scenario: Validate warning messages for Subject only others to be filled`, async () => {
    ...
    try {
      ...
      // Delete email because it is automatically filled with the user email
      await page.locator('#email').fill(reseller.contactUs?.contactUsEmail);
      const emailValue = await page.locator('#email').inputValue();
      console.log('Email field is empty', emailValue);

      // Subject field is empty
      await page.locator('#subject').fill('');
      const subjectValue1 = await page.locator('#subject').inputValue();
      console.log('Subject field has not value', subjectValue1);

      // Fill the textarea with the text
      await page.locator('#issue').fill(reseller.contactUs?.contactUsTextArea);
      const issueValue = await page.locator('#issue').inputValue();
