...
async function loginScreen(page, reseller, stepNumber) {
  ...
  try {
    ...
    stepNumber += 1;
    await loginScreenSecondScenario(page, reseller, stepNumber);

    stepNumber += 1;
    await loginScreenThirdScenario(page, reseller, stepNumber);

    stepNumber += 1;
    await loginScreenFourScenario(page, reseller, stepNumber);

    stepNumber += 1;
    await loginScreenFiveScenario(page, reseller, stepNumber);

    stepNumber += 1;
    await loginScreenSixScenario(page, reseller, stepNumber);
...
async function loginScreenSecondScenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Login Screen Second Scenario: Click from the haeader need to redirect to the login screen also check are exist all fields`, async () => {
    ...
    try {
      ...
      await checkLoginButtonDisabled(page, reseller);
...
async function loginScreenThirdScenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Login Screen Third Scenario: Fill Email and password which is not exist in the Base need to appear pop up`, async () => {
    ...
    try {
      // Fill the Email 
      await page.locator('#username').fill(reseller.login.email);
...
async function loginScreenFourScenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Login Screen Four Scenario: Fill Email Password need to be Empty Login button need to be disabled`, async () => {
    ...
    try {
      ...
      await page.waitForSelector('#login-button', { state: 'visible', timeout: 50000 });
...
async function loginScreenFiveScenario(page, reseller, stepNumber) {
...
async function loginScreenSixScenario(page, reseller, stepNumber) {
  await test.step(`${stepNumber}. Login Screen Six Scenario:Successfully logged user Also check if appear Category Details screen etc  `, async () => {
    ...
    try {
      ...
      //When is logged is redirect to the Profile Page
      if (reseller.selectProfile){
       ...
       // Click the profile card and wait for home page navigation
        await Promise.all([
            page.waitForURL(`https://${reseller.name}-v3-dev.streann.tech/`, { timeout: 20000 }),
      ...
      stepNumber += 1;
      await UserDetailsScreen(page, reseller, stepNumber,'loggedUser');

      //if(action === 'panamsport' || action === 'prtv' || action === 'tdmax' || action === 'tvin' || action === 'gamestreammedia') {
      if (reseller.checkPlayerScreen) {
        stepNumber += 1;
        await checkPlayerScreen(page, reseller, stepNumber);
      }
...
async function checkLoginButtonDisabled(page, reseller, enabled) {
  if (enabled) {
    // Should be enabled
...
async function loginScreenNewPassword(page, reseller, stepNumber) {
  ...
  try {
      ...
      const baseUrl1 = `https://${reseller.name}-v3-dev.streann.tech/login`;
      console.log(`⏳ Navigating to: ${baseUrl1}`);

      // Use 'domcontentloaded' for faster, more reliable loading (with retry logic)
      let response;
      let retries = 3;

      while (retries > 0) {
        try {
          response = await page.goto(baseUrl1, {
          ...
          retries--;
          console.log(`⚠️ Navigation attempt failed: ${err.message}`);

          if (retries === 0) {
            console.error('❌ All navigation retries exhausted');
            throw new Error(`Failed to navigate to ${baseUrl1} after multiple attempts: ${err.message}`);
          }
      ...
      // For PRTV, add extra wait time before checking for login button
      if (reseller.name === 'prtv') {
        console.log('⏳ PRTV detected: adding extra wait time before login button check...');
        await page.waitForTimeout(5000);
      } else {
        // For other resellers, add a small buffer
        await page.waitForTimeout(2000);
      }

      const loginButton = page.locator('#login-button, [type="submit"]').first();
      // Wait for the login button to be visible with extended retries
      try {
        await expect(loginButton).toBeVisible({ timeout: 30000 });
        console.log('✅ Login button is visible');
      } catch (err) {
        console.log('First wait failed, retrying with longer timeout...');
        await page.waitForTimeout(5000); // wait extra 5s
        try {
          await expect(loginButton).toBeVisible({ timeout: 20000 });
          console.log('✅ Login button is visible after retry');
        } catch (retryErr) {
          console.log('❌ Login button still not visible. Current page state:');
          console.log('URL:', page.url());
          console.log('Page title:', await page.title());
          throw new Error(`Login button not found after multiple retries: ${retryErr.message}`);
        }
      }
      ...
      if (reseller.selectProfile) {
        // After Login, be sure that it redirects to the Profile Page
        const profileUrl = `https://${reseller.name}-v3-dev.streann.tech/profile/select-profile`;
        await page.waitForURL(profileUrl, { timeout: 50000 , waitUntil: 'domcontentloaded' });
        await expect(page).toHaveURL(profileUrl);
        console.log(`✅ Successfully navigated to ${profileUrl}`);
